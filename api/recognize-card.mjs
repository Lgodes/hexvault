...
export default async function handler(req,res){
  ...
  const mode=req.body?.mode==='binder'?'binder':'single',images=Array.isArray(req.body?.images)?req.body.images.slice(0,12).map(String):[],image=String(req.body?.image||''),setCodes=Array.isArray(req.body?.sets)?req.body.sets.slice(0,8):[],language=String(req.body?.language||'auto');
  if(mode==='binder'){
    if(!images.length||images.some(value=>!value.startsWith('data:image/')||value.length>700_000))return json(res,400,{error:'Compressed binder pocket images are required.'});
    const prompt=`Identify the Magic: The Gathering card in each numbered binder-pocket image. A pocket may be empty or unreadable. Cards may use any official MTG language. Return one result for every slot, in the same order. canonical_name must be the official English card name; language must be an MTG/Scryfall language code; confidence is 0-100. Do not invent a result for an empty or unreadable pocket. ${language!=='auto'?`The expected language is ${language}, but correct it when the image clearly differs.`:''} ${setCodes.length?`Likely set codes are ${setCodes.join(', ')}, but do not force them.`:''}`;
    const content=[{type:'input_text',text:prompt}];images.forEach((value,index)=>{content.push({type:'input_text',text:`Binder slot ${index+1}`},{type:'input_image',image_url:value,detail:'low'})});
    const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),10000);
    try{
      const response=await fetch('https://api.openai.com/v1/responses',{method:'POST',signal:controller.signal,headers:{Authorization:`Bearer ${process.env.OPENAI_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({model:process.env.OPENAI_VISION_MODEL||'gpt-4o-mini',input:[{role:'user',content}],text:{format:{type:'json_schema',name:'mtg_binder_recognition',strict:true,schema:{type:'object',additionalProperties:false,properties:{cards:{type:'array',minItems:images.length,maxItems:images.length,items:{type:'object',additionalProperties:false,properties:{slot:{type:'integer'},recognized:{type:'boolean'},canonical_name:{type:'string'},printed_name:{type:'string'},language:{type:'string'},set_code:{type:'string'},collector_number:{type:'string'},confidence:{type:'integer'}},required:['slot','recognized','canonical_name','printed_name','language','set_code','collector_number','confidence']}}},required:['cards']}}},max_output_tokens:1300})});
      ...
      const parsed=parseModelJSON(payload),verified=await Promise.all((parsed.cards||[]).map(async clue=>{if(!clue.recognized)return {slot:clue.slot,recognized:false};const result=await verify(clue);return result.card?{slot:clue.slot,recognized:true,card:result.card,detected:{...clue,language:result.lang||languageCode(clue.language)||result.card.lang||''}}:{slot:clue.slot,recognized:false}}));
      return json(res,200,{cards:verified,verified:true});
    }catch(error){if(error.name==='AbortError')return json(res,504,{error:'Binder recognition took too long. Try fewer pockets or move closer.'});return json(res,500,{error:error.message||'Binder recognition failed.'})}finally{clearTimeout(timer)}
  }
